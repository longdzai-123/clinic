...
public interface RemedyDetailsRepo extends JpaRepository<RemedyDetails, Integer> {
    ...
    @Query(value = "SELECT rd FROM remedy_details rd WHERE rd.remedy_id=:remedyId", nativeQuery = true)
    List<RemedyDetails> getByRemedyId(Integer remedyId);
    @Modifying
    @Query(value = "DELETE FROM remedy_details rd WHERE rd.remedy_id=:remedyId", nativeQuery = true)
